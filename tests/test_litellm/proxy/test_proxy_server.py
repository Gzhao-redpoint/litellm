...
async def test_reseed_spend_from_db_skips_window_variant_keys():
    ...
    ps.prisma_client = fake_prisma
    try:
        assert await _reseed_spend_from_db("spend:key:sk-abc:window:1h") is None
        assert await _reseed_spend_from_db("spend:team:team-1:window:1d") is None
...
async def test_reseed_locks_dict_is_bounded():
    """
    `_reseed_locks` is an LRU bounded at `_RESEED_LOCKS_MAX_SIZE` to prevent
    unbounded growth in long-lived deployments with high counter-key churn.
    Inserting more than the cap evicts the oldest entries.
    """
    import litellm.proxy.proxy_server as ps
    from litellm.proxy.proxy_server import _get_reseed_lock

    orig = ps._reseed_locks.copy()
    ps._reseed_locks.clear()
    orig_max = ps._RESEED_LOCKS_MAX_SIZE
    ps._RESEED_LOCKS_MAX_SIZE = 5
    try:
        for i in range(7):
            await _get_reseed_lock(f"spend:key:test-key-{i}")
        assert len(ps._reseed_locks) == 5, f"got {len(ps._reseed_locks)}"
        # Oldest two evicted
        assert "spend:key:test-key-0" not in ps._reseed_locks
        assert "spend:key:test-key-1" not in ps._reseed_locks
        # Most recent retained
        assert "spend:key:test-key-6" in ps._reseed_locks
    finally:
        ps._RESEED_LOCKS_MAX_SIZE = orig_max
        ps._reseed_locks.clear()
        ps._reseed_locks.update(orig)
